        ...
        if prompt_path.exists():
            try:
                with open(prompt_path, "r", encoding="utf-8") as f:
                    logger.info("Loaded prompt template from file: %s", prompt_path)
                    return f.read()
            except Exception as e:
                logger.warning("Failed to load prompt template from file: %s", e)
                return self._get_fallback_prompt()
        else:
            logger.warning("Prompt template file not found at: %s, using fallback", prompt_path)
            return self._get_fallback_prompt()
...
class PubMedQueryExecutor:
    ...
    def execute_query(self, query: str, output_dir: str) -> str:
        ...
        """
        logger.info("Executing PubMed query: %s", query)
        ...
        if not self._check_edirect_available():
            ...
            raise RuntimeError("EDirect tools not found. Please install NCBI EDirect tools or provide a MEDLINE file directly.")
        ...
        try:
            ...
            with open(medline_file, 'w', encoding='utf-8') as f:
                ...
                if result.returncode != 0:
                    logger.error("Query execution failed with return code %d: %s", 
            ...
            if file_size == 0:
                ...
                raise RuntimeError("No results found for the query or query execution failed")
            
            logger.info("Query results saved to: %s (size: %d bytes)", medline_file, file_size)
            return medline_file
            
        except Exception as e:
            logger.exception("Error executing PubMed query: %s", e)
            raise
    ...
    def _check_edirect_available(self) -> bool:
        ...
        try:
            ...
            logger.warning("EDirect tools not available: %s", e)
            return False
...
class MedlineParser:
    ...
    def parse_medline_file(self, file_path: str) -> pd.DataFrame:
        ...
        """
        logger.info("Parsing MEDLINE file: %s", file_path)
        records = []
        ...
        try:
            with open(file_path, 'r', encoding='utf-8') as handle:
                ...
                for record in medline_records:
                    ...
                    if not title and not abstract:
                        skipped_count += 1
                        logger.info("Skipping record %s: no title or abstract", pmid)
                        continue
            ...
            logger.info("Parsed %d records from MEDLINE file (%d skipped, %d valid)", 
                       record_count, skipped_count, len(df))
            ...
            logger.exception("Error parsing MEDLINE file: %s", e)
            raise
...
class LLMTargetExtractor:
    ...
    def __init__(self, config: TargetMiningConfig):
        ...
        logger.info("LLMTargetExtractor initialized with model: %s", config.model)
    
    def extract_targets_from_dataframe(self, df: pd.DataFrame, output_dir: str) -> pd.DataFrame:
        """
        ...
        """
        logger.info("Starting target extraction from DataFrame with %d records", len(df))
        ...
        logger.info("Processing %d records in %d batches (batch size: %d)", 
                   total_rows, num_batches, self.config.batch_size)
        ...
        for batch_num in range(num_batches):
            ...
            if os.path.exists(batch_file):
                logger.info("Batch %d/%d already processed, skipping", batch_num + 1, num_batches)
                successful_batches += 1
                continue
            
            logger.info("Processing batch %d/%d (rows %d-%d)", 
                       batch_num + 1, num_batches, start_idx, end_idx-1)
            ...
            try:
                ...
                if not result_batch.empty:
                    ...
                    logger.info("Batch %d completed with %d results", batch_num + 1, len(result_batch))
                    successful_batches += 1
                else:
                    logger.warning("Batch %d produced no results", batch_num + 1)
                    
            except Exception as e:
                logger.error("Error processing batch %d: %s", batch_num + 1, e)
                continue
        
        logger.info("Batch processing completed: %d/%d batches successful", 
    ...
    def _process_batch(self, batch_df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info("Processing batch with %d records", len(batch_df))
        all_results = []
        ...
        for _, row in batch_df.iterrows():
            try:
                ...
                if result is not None and not result.empty:
                    ...
                    logger.info("No targets extracted from PMID: %s", row.get('PMID', 'unknown'))
                    failed_records += 1
                    
            except Exception as e:
                logger.error("Error processing PMID %s: %s", row.get('PMID', 'unknown'), e)
                failed_records += 1
                continue
        
        logger.info("Batch processed: %d successful, %d failed", successful_records, failed_records)
    ...
    def _extract_targets_from_record(self, row: pd.Series) -> Optional[pd.DataFrame]:
        ...
        logger.info("Extracting targets from PMID: %s", pmid)
        ...
        for attempt in range(self.config.max_retries):
            try:
                logger.info("Making LLM API call for PMID %s (attempt %d)", pmid, attempt + 1)
                ...
                if answer:
                    ...
                    if not result_df.empty:
                        logger.info("Extracted %d targets from PMID: %s", len(result_df), pmid)
                        return result_df
                    else:
                        logger.info("No targets parsed from LLM response for PMID: %s", pmid)
                else:
                    logger.info("Empty response from LLM for PMID: %s", pmid)
                
                break
                
            except Exception as e:
                logger.warning("API call failed for PMID %s, attempt %d: %s", 
                             pmid, attempt + 1, e)
                if attempt < self.config.max_retries - 1:
                    logger.info("Retrying after delay of %d seconds", self.config.retry_delay)
                    time.sleep(self.config.retry_delay)
                else:
                    logger.error("Failed to process PMID %s after %d attempts", 
                               pmid, self.config.max_retries)
    ...
    def _parse_llm_response(self, response: str) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info("Found %d table rows in LLM response", len(data))
            table_data_str = '\n'.join(data)
            ...
            logger.info("Successfully parsed %d targets from LLM response", len(table_data))
            return table_data
            
        except Exception as e:
            logger.error("Error parsing LLM response: %s", e)
            logger.info("Response content: %s", response[:500] + "..." if len(response) > 500 else response)
            return pd.DataFrame()
    
    def _combine_batch_results(self, batch_output_dir: str, num_batches: int) -> pd.DataFrame:
        """Combine all batch results into final DataFrame"""
        logger.info("Combining batch results from %d batches", num_batches)
        all_batches = []
        loaded_batches = 0
            ...
            if os.path.exists(batch_file):
                try:
                    ...
                    if not batch_df.empty:
                        ...
                        loaded_batches += 1
                        logger.info("Loaded batch %d with %d records", batch_num + 1, len(batch_df))
                    else:
                        logger.info("Batch %d is empty", batch_num + 1)
                except Exception as e:
                    logger.error("Error reading batch file %s: %s", batch_file, e)
        
        if all_batches:
            final_df = pd.concat(all_batches, ignore_index=True)
            logger.info("Combined %d batches into %d total records", loaded_batches, len(final_df))
            return final_df
        else:
...
class TargetMiningTool:
    ...
    def mine_targets(self, 
                    query: str, 
                    output_dir: str = None,
                    medline_file: str = None) -> Tuple[List[str], pd.DataFrame]:
        """
        ...
        try:
            ...
            if medline_file and os.path.exists(medline_file):
                logger.info("Using provided MEDLINE file: %s", medline_file)
            ...
            logger.info("Parsed records saved to: %s", parsed_file)
            
            # Step 3: Extract targets using LLM
            logger.info("Extracting targets using LLM...")
            results_df = self.llm_extractor.extract_targets_from_dataframe(df, output_dir)
            ...
            logger.info("Total unique targets found: %d", len(gene_list))
            logger.info("Results saved to: %s", output_dir)
            
            return gene_list, final_df
            
        except Exception as e:
            logger.exception("Target mining process failed: %s", e)
            raise
    
    def _process_final_results(self, results_df: pd.DataFrame, output_dir: str) -> Tuple[List[str], pd.DataFrame]:
        """Process and clean final results"""
        logger.info("Processing final results (%d initial records)", len(results_df))
        
        # Save raw results
        raw_results_file = os.path.join(output_dir, "raw_extraction_results.csv")
        results_df.to_csv(raw_results_file, index=False)
        logger.info("Raw results saved to: %s", raw_results_file)
        
        # Clean and deduplicate if configured
        if self.config.deduplicate and 'Target/Molecule(Name only)' in results_df.columns:
            ...
            final_df = results_df.drop_duplicates(
                subset=['Target/Molecule(Name only)'], 
                keep='first'
            )
            
            logger.info("Deduplication: %d -> %d -> %d records (initial -> cleaned -> deduplicated)", 
                       initial_count, cleaned_count, len(final_df))
        else:
            final_df = results_df
        ...
        if 'Target/Molecule(Name only)' in final_df.columns:
            ...
            logger.info("Generated gene list with %d entries", len(gene_list))
        else:
            gene_list = []
            logger.warning("No 'Target/Molecule(Name only)' column found for gene list generation")
        ...
        logger.info("Gene list saved to: %s", gene_list_file)
        logger.info("Final results saved to: %s", final_results_file)
