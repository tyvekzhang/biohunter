...
class FileServiceImpl(BaseServiceImpl[FileMapper, FileModel], FileService):
    ...
    async def upload_chunk(
        self, upload_id: str, chunk_number: int, chunk_hash: str, file: UploadFile
    ) -> ChunkUploadResponse:
    ...
    async def merge_chunks(self, upload_id: str) -> MergeChunksResponse:
        ...
        try:
            ...
            if len(metadata["uploaded_chunks"]) != metadata["total_chunks"]:
                missing_chunks = set(range(metadata["total_chunks"])) - set(
                    metadata["uploaded_chunks"]
                )
                raise HTTPException(
                    status_code=400, detail=f"Missing chunks: {sorted(missing_chunks)}"
                )
            ...
            with open(final_path, "wb") as outfile:
                for chunk_number in range(metadata["total_chunks"]):
                    chunk_path = os.path.join(
                        temp_upload_dir, f"chunk_{chunk_number:06d}"
                    )
            ...
            if final_hash != metadata["file_hash"]:
                ...
                raise HTTPException(
                    status_code=500, detail=f"File SHA-256 mismatch after merge"
                )
            ...
            if file_record:
                ...
                await self.mapper.update_by_id(data=file_record)
    ...
    async def use_user_conversation_files(
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to copy files: {str(e)}"
            )
    ...
    async def import_local_file_to_uploads(
        ...
        try:
            ...
            if not os.path.exists(local_file_path):
                raise HTTPException(
                    status_code=404, detail=f"Local file not found: {local_file_path}"
                )
            ...
            if existing_file:
                logger.info(
                    f"File with hash {file_hash} already exists, returning existing record"
                )
                return existing_file
            ...
            if not os.path.exists(final_path):
                raise HTTPException(
                    status_code=500, detail="Failed to copy file to upload directory"
                )
            ...
            if copied_file_hash != file_hash:
                os.remove(final_path)
                raise HTTPException(
                    status_code=500, detail="File integrity check failed after copy"
                )
            ...
            await self.mapper.insert(data=file_model)
            logger.info(
                f"Created file record with state=2 for imported file: {file_uuid}"
            )
            ...
            if "final_path" in locals() and os.path.exists(final_path):
                try:
                    ...
                    pass
            raise HTTPException(
                status_code=500, detail=f"Failed to import local file: {str(e)}"
            )
